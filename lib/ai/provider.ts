...
export async function runAiInvestigation(input: AiInvestigationInput): Promise<AiExecutionResult> {
  ...
  const endpoint = process.env.AI_ENDPOINT || process.env.AI_BASE_URL || process.env.OPENAI_BASE_URL || 'https://api.openai.com/v1';
  const model = process.env.AI_MODEL || 'gpt-4o-mini';

  return requestAiInvestigation(input, apiKey, endpoint, model, fetch, 10000);
}
