...
function getHypothesisStatusSummary(hypo: InvestigationHypothesis): string {
  if (hypo.status === 'SUPPORTED' || hypo.status === 'PLAUSIBLE') {
    if (
...
export function InvestigationsView({
  ...
  return (
    <section className="panel investigations-view" aria-label="Investigation workspace">
      ...
      {loading && (
        <div className="investigation-loading-panel" style={{ padding: '40px 24px', maxWidth: '640px', margin: '0 auto' }}>
      ...
      {report && !loading && (
        <div className="investigation-report" style={{ padding: '24px 30px' }}>
          {/* Report Top Strip */}
            ...
            return (
              <div style={{ background: 'var(--surface-raised)', border: '1px solid var(--border)', borderRadius: '8px', padding: '20px 24px', marginBottom: '28px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px', marginBottom: '14px' }}>
                  <span className="eyebrow" style={{ margin: 0 }}>Executive Assessment &amp; Verdict</span>
                  <span
                    className="badge"
                    style={{
          ...
          {/* Section: Deterministic Signals Grid */}
          ...
          {/* Section: Market Catalyst & News Evidence */}
          <div style={{ marginBottom: '32px' }}>
            <div className="section-title" style={{ marginBottom: '16px' }}>
              <h3 style={{ fontSize: '13.5px', fontWeight: 650, letterSpacing: '0.4px', textTransform: 'uppercase', color: 'var(--muted)' }}>
          ...
          <div style={{ marginBottom: '32px' }}>
            ...
            <p style={{ fontSize: '12px', color: 'var(--muted)', marginBottom: '16px', lineHeight: '1.4' }}>
              Confidence scores represent rule-based heuristic weights rather than empirical statistical probabilities.
            </p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {report.hypotheses.map(hypo => {
                const isSupported = hypo.status === 'SUPPORTED';
                const isPlausible = hypo.status === 'PLAUSIBLE';
                const isUnresolved = hypo.status === 'UNRESOLVED';
                ...
                return (
                  <div
                    ...
                    style={{
                      background: 'var(--surface)',
                      border: `1px solid ${isSupported ? 'var(--brand-border)' : 'var(--border)'}`,
                      borderRadius: '8px',
                      padding: '14px 18px',
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
                      ...
                      <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }} title={`Heuristic confidence: ${Math.round(hypo.confidence * 100)}%`}>
                          ...
                          <div style={{ width: '40px', height: '4px', background: 'var(--border)', borderRadius: '2px', overflow: 'hidden' }}>
                            <div style={{ width: `${Math.round(hypo.confidence * 100)}%`, height: '100%', background: isSupported ? 'var(--accent)' : isPlausible ? 'var(--amber, #d97706)' : 'var(--muted)' }} />
                          </div>
                        </div>
                        <span
                          className="badge"
                          style={{
                            background: isSupported ? 'var(--accent-surface)' : isPlausible ? 'var(--amber-surface)' : 'var(--surface-raised)',
                            color: isSupported ? 'var(--accent)' : isPlausible ? 'var(--amber)' : isUnresolved ? 'var(--faint)' : 'var(--muted)',
                            border: '1px solid currentColor',
                            padding: '2px 8px',
                            borderRadius: '5px',
                            fontSize: '10.5px',
                    ...
                    {isExpanded && (
                      ...
                      >
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '14px', background: 'var(--surface-raised)', padding: '12px 16px', borderRadius: '6px' }}>
                          <div>
          ...
          {/* Section: Market Evidence Microstructure Snapshot */}
          <div style={{ marginBottom: '32px' }}>
            <div className="section-title" style={{ marginBottom: '16px' }}>
              <h3 style={{ fontSize: '13.5px', fontWeight: 650, letterSpacing: '0.4px', textTransform: 'uppercase', color: 'var(--muted)' }}>
          ...
          <div style={{ background: 'var(--surface-raised)', border: '1px solid var(--border)', borderRadius: '8px', padding: '16px 20px' }}>
      ...
      <div className="provider-grid">
        ...
        <section className="panel provider-card">
          <div className="provider-title">
            ...
            <StatusBadge status={error ? 'ERROR' : 'LIVE'} />
          </div>
          <dl className="property-list">
...
export function SystemStatusView({ assets, data, error }: { assets: MarketAsset[]; data: MarketResponse; error: string | null }) {
  ...
  const checks = [
    ...
    { label: 'News feed', description: 'Benzinga syndicated real-time equity news via Alpaca Data API', status: error ? 'ERROR' as const : 'LIVE' as const, detail: 'Bounded 48h search window · Ticker-filtered events' },
    { label: 'AI engine', description: 'Multi-hypothesis research synthesis and evaluation', status: error ? 'ERROR' as const : 'LIVE' as const, detail: error ? 'Service diagnostics unavailable' : 'Deterministic heuristics active &middot; AI synthesis ready' },
